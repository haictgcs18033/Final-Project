    ...
    if (cate._id === parentId) {
      ...
      newCategoryArray.push({
        ...
        children: cate.children
          ? setNewCategory(cate.children, category, parentId)
          : [],
      });
    }
  }
...
export const adminReducer = (state = stateDefault, action) => {
  switch (action.type) {
    ...
    case "CHOOSE_COLOR": {
      ...
      let index = productUpdate.color?.findIndex(
    ...
    case "DELETE_FILE": {
      ...
      let index = productPicturesUpdate.productPictures.findIndex(
        (img) => img.name === action.data
      );
      if (index !== -1) {
    ...
    case "ADD_CATEGORY": {
      ...
      let updateCategory = setNewCategory(
        state.categoryList,
        category,
        category.parentId
      );
    ...
    case "GET_ADMIN_DETAIL": {
      return {
        ...
        userImage: action.adminDetail.profilePicture,
      };
    }
